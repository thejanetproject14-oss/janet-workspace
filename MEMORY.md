# MEMORY.md - Janet's Long-Term Memory
# Trimmed Mar 27. Archive: memory/MEMORY-ARCHIVE.md. Search there for old details.

## Protocols & Behaviors
- **"Task:" protocol** -- immediately log to Notion master task list (30beab20-0eed-8188-8965-d32e0f59b930). No questions.
- **GOOD4ME / Grateful:** -- append to Notion page 31beab20-0eed-8113-a010-fe6776857a26. Format: [Date] -- [what] [emoji].
- **Morning digest** = Analemma only. NEVER fabricate. Pull from Notion Open Tasks + Linear.
- **Autonomous:** Draft social, research, update Shopify, log tasks, run agents, message Sita (group only).
- **Draft first:** Influencer outreach emails (personal voice, no AI phrasings).
- **Always ask:** Spending money, posting publicly, anything irreversible.
- **NEVER DM Sita.** Group only. NEVER respond to Sita's DMs or Tiny Wins members' DMs.

## Current Phase: Content Seeding (Mar 16 - Apr 15)
Website LIVE at analemma.shop. Priority: traffic, audience, conversion.
- Influencer outreach (micro, SG first). 30 PR boxes. Lock box concept.
- Content pillars: founder story, product education, BTS, UGC.
- Unposted photoshoot content available. Bridgerton + postman videos need editing.
- AI content pipeline: HeyGen + ElevenLabs + Kling + Blotato (~$55/mo). Setup pending.

## Top 7 (set Mar 24)
1. TikTok Shop setup
2. Bridgerton video edit
3. Perfume packaging + skin analysis app
4. Clear baggage + stocks
5. Anchorpoint pop-up (Mar 29-30, Vageesh covering)
6. Artbox pop-up (April TBC)
7. Warehouse situation

## Upcoming Dates
- **Mar 28:** Perfume packaging orders DEADLINE
- **Mar 29:** AI x Commerce Hackathon (Sunshine attending). Build: Skin Analysis → Product Recommender.
- **Mar 29-30:** Anchorpoint pop-up (Vageesh solo)
- **April:** Artbox (TBC)
- **May 21-30:** Seoul (solo + COSMOBEAUTY May 27-29). Book flights THIS WEEK (Kris+ 10% expires Mar 31).
- **May 31-Jun 10:** Bangalore
- **Jul 4-5:** Parents' Shashtiaptapoorti (NON-NEGOTIABLE)

## Health (Active)
- Thyroid #1 priority. Protocol built: protocols/thyroid-recovery.md + Notion (32feab20-0eed-8178-ba90-c2eb1bb78741).
- Stack: Selenium (2 Brazil nuts), Zinc, Vit D, Mg glycinate, Ashwagandha KSM-66, Myo-inositol+Se.
- First step: BLOODWORK. Task in Notion.
- Allergic rhinitis from Malassezia (dandruff) linked to hypothyroidism. Nizoral 2-3x/week.
- Wound: electrocautery big toe Mar 22. Verrumal bottle BROKE -- get replacement from GP/polyclinic. No yoga until Mar 30.
- Health points game: 200 pts, streak 0. 10pm daily check-in. Full rules in memory/health-game.md.

## Fundraise
- Angel ($200-500K) Aug-Nov 2026. Series A ($8M SGD) Jun-Aug 2027.
- 85-95% gross margins. Personal/business finances SEPARATE.

## Financial
- Cash-strapped. Clear personal loans by EOY. Strict budgeting.
- DBS: 65K points. Seoul flights = 44,500 pts. Use strategically.
- Financial planning session logged for next week.

## Personal
- Relationship: planning to exit. Details in daily files ONLY. NEVER surface externally.
- HDB MOP Nov 2028. Turns 35 Jul 2029. Deed of separation first.

## OpenClaw
- Version 2026.3.24. Lossless-claw install FAILED -- DO NOT re-attempt without maintenance window. Use `openclaw plugins install --link` not manual config edits.
- Memory-LanceDB Pro available (CortexReach). Not yet installed.
- Vercel backoffice: https://analemma-backoffice.vercel.app/

## Media Pitch Kit (completed Mar 26)
- Notion: https://www.notion.so/Media-Pitch-Kit-SG-Publications-32feab200eed81c2a3d3f84a22fb579d
- 5 pitches: Daily Vanity (Kristen Juliet Soh), Her World (Letty Seah), ZULA, Vulcan Post, NYLON SG.
